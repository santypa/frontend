  ...
  <container class="container">
    ...
    <div class="con text-center m-4 p-4">
      ...
      >
        ...
        <div class="row">
          <div
            class="row col-12 col-sm-6 col-md-4 col-lg-4 col-xl-3"
            style="border-radius: 10%"
            v-for="forma in formato"
            :key="forma.id"
          >
            <div class="card m-1">
              ...
              <div class="card-body">
                <img
                  :src="'http://localhost:1337' + forma.formato.url"
                  id="image"
                  @click="formatoes(forma.id)"
                  class="card-img-top"
                />
              </div>
            </div>
        ...
        <button type="submit" class="row bg-dark text-white m-4">
          Registrar
        </button>
      </form>
    </div>
...
export default {
  ...
  methods: {
    ...
    prevista(e) {
      this.imageprevista = URL.createObjectURL(e.target.files[0]);
    ...
    cambiarArchivo(e) {
      ...
      axios.get("http://localhost:1337/formatoes").then((res) => {
        ...
        if (res.data.length == 0) {
          ...
          for (var t = 0; t <= res.data.length; t++) {
            ...
            if (this.nam == res.data[t].user["username"]) {
              (this.empre = 2), (t = res.data.length);
    ...
    guardar() {
      ...
      if (this.empre == 3) {
        axios
          .post("http://localhost:1337/empresas", form, {
            headers: {
